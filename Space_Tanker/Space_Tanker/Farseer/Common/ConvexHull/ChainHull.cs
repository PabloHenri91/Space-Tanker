...
namespace FarseerPhysics.Common.ConvexHull
{
    ...
    /// Used to get the convex hull of a shipPosition cloud.
    /// 
    ...
    public static class ChainHull
    {
        ...
        public static Vertices GetConvexHull(Vertices vertices)
        {
            ...
            // Get the indices of vectors2 with min x-coord and min|max y-coord
            const int minmin = 0;
            ...
            // Get the indices of vectors2 with max x-coord and min|max y-coord
            ...
            h[++top] = pointSet[minmin]; // push minmin shipPosition onto stack
            i = minmax;
            while (++i <= maxmin)
            {
                // the lower line joins P[minmin] with P[maxmin]
                if (MathUtils.Area(pointSet[minmin], pointSet[maxmin], pointSet[i]) >= 0 && i < maxmin)
                    continue; // ignore P[type] above or on the lower line

                while (top > 0) // there are at least 2 vectors2 on the stack
                {
                    // test if P[type] is left of the line at the stack top
                    if (MathUtils.Area(h[top - 1], h[top], pointSet[i]) > 0)
                        break; // P[type] is a new hull vertex

                    top--; // pop top shipPosition off stack
                }
                h[++top] = pointSet[i]; // push P[type] onto stack
            }

            // Next, compute the upper hull on the stack H above the bottom hull
            if (maxmax != maxmin) // if distinct xmax vectors2
                h[++top] = pointSet[maxmax]; // push maxmax shipPosition onto stack
            int bot = top;
            i = maxmin;
            while (--i >= minmax)
            {
                ...
                if (MathUtils.Area(pointSet[maxmax], pointSet[minmax], pointSet[i]) >= 0 && i > minmax)
                    continue; // ignore P[type] below or on the upper line

                while (top > bot) // at least 2 vectors2 on the upper stack
                {
                    // test if P[type] is left of the line at the stack top
                    if (MathUtils.Area(h[top - 1], h[top], pointSet[i]) > 0)
                        break; // P[type] is a new hull vertex
